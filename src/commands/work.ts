...
export async function work(storyCode: string): Promise<void> {
  ...
  if (story.status === 'done') {
    console.log(chalk.yellow(`⚠  Warning: story ${storyCode} is already done.`));
    console.log(chalk.dim('  No changes made. Use pm story update to change status.'));
    return;
  }

  // Print warning if already in_progress, but continue
  if (story.status === 'in_progress') {
    console.log(chalk.yellow(`⚠  Warning: story ${storyCode} is already in_progress.`));
  }
  ...
  const projectYaml = path.join(getProjectsDir(), parsed.projectCode, 'project.yaml');
  ...
  console.log(chalk.bold('  Epic:    ') + epic.code + chalk.dim(' — ') + epic.title);
  console.log(chalk.bold('  Project: ') + parsed.projectCode + chalk.dim(' — ') + projectName);
  console.log(
    chalk.bold('  Status:  ') +
