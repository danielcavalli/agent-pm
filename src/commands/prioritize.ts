...
export async function prioritize(
  ...
  if (!fs.existsSync(projectYaml)) {
    throw new ProjectNotFoundError(getProjectCode() || "PROJECT");
  }
  ...
  if (opts.epic) {
    const epicCode = resolveEpicCode(opts.epic);
    ...
    console.log("");
    console.log(
      chalk.bold("  Epic:     ") + epicCode + chalk.dim(" — ") + epic.title,
    );
    ...
    if (inProgress.length > 0) {
      ...
      inProgress.forEach((s, i) => {
        console.log(
          `    ${i + 1}. ${chalk.yellow(STATUS_ICON["in_progress"] ?? "●")} ${chalk.bold(s.code)} — ${s.title} [${s.priority}] (${s.story_points}pts)`,
        );
      });
      console.log("");
    }
    ...
    if (backlog.length > 0) {
      ...
      backlog.forEach((s, i) => {
        console.log(
          `    ${i + 1}. ${chalk.dim(STATUS_ICON["backlog"] ?? "○")} ${chalk.bold(s.code)} — ${s.title} [${s.priority}] (${s.story_points}pts)`,
        );
      });
      console.log("");
    }
    ...
    console.log(
      `  You are re-prioritizing stories in epic ${epicCode} (${epic.title}).`,
    );
    console.log(`  Strategy: "${strategy}"`);
    console.log("");
    console.log("  Re-order the backlog stories in the epic YAML file by:");
    console.log(
      "    1. Updating the `priority` field of each story (high/medium/low)",
    );
    console.log(
      "    2. Reordering the stories array so high-priority items appear first",
    );
    console.log("    3. Keeping in_progress/done/cancelled stories in place");
    console.log("");
    console.log(
      "  After re-ordering, explain your reasoning for the new order.",
    );
    console.log(chalk.cyan.bold("━".repeat(72)));
    console.log("");
    return;
  ...
  for (const epicFile of epicFiles) {
    try {
      const epic = readYaml(epicFile, EpicSchema);
      const backlog = (epic.stories ?? []).filter(
        (s) => s.status === "backlog",
      );
      if (backlog.length === 0) continue;

      console.log(
        `  ${chalk.bold(epic.code)} — ${epic.title} [${epic.priority}]`,
      );
      backlog.forEach((s, i) => {
        console.log(
          `    ${i + 1}. ${chalk.dim("○")} ${chalk.bold(s.code)} — ${s.title} [${s.priority}] (${s.story_points}pts)`,
        );
      });
      console.log("");
    } catch {
  ...
  console.log(
    `  You are re-prioritizing the backlog for project ${projectCode}.`,
  );
  console.log(`  Strategy: "${strategy}"`);
  console.log("");
  console.log("  Re-order and re-prioritize by:");
  console.log(
    "    1. Updating `priority` fields on stories and epics in the YAML files",
  );
  console.log(
    "    2. Reordering story arrays within each epic (high priority first)",
  );
