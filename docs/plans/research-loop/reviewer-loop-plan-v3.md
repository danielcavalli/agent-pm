...
**Status: COMPLETE** -- v3 implementation finished 2026-03-15. All v3 features implemented in `install/commands/pm-review-plan.md` (1382 lines). All 764 tests pass. Help registration updated to reflect Creative Agent. install.sh glob coverage verified.
...
- `--verbose` -- full per-agent output displayed to the user after each agent dispatch (default behavior in v3)
...
| Mode | Flag | Behavior |
| --- | --- | --- |
| **Verbose** (default) | `--verbose` or omitted | Full agent output displayed after each agent dispatch. All structured sections (claims inventory, bibliography, score tables, pointers, integrity audit, revision changelog) shown in full. This is the current v3 behavior. |
